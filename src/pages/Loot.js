  ...
  useEffect(() => {
    if (filteredItems.length > 0) {
      gsap.fromTo(
        ".loot-item",
        { opacity: 0 },
        { duration: 0.3, opacity: 1, stagger: 0.03 }
      );
    }
  }, [filteredItems]);
  ...
  return (
    <Container>
      <Row className="mb-5">
        <Navbar
          sticky="top"
          className="w-100 p-0 theme1-backer"
          id="sticky-filter"
        >
        ...
        {filteredItems.length > 0 || loading ? null : (
          <Col
            xs={12}
            className="pt-4 pb-4 pl-5 pr-5 background-unset add-background-dark"
          >
            <p className="text-center font-weight-bold text-light">
              Tap Party Gold to expand. Color tags can be edited to suit your
              party's needs.
            </p>
            <p className="text-center font-weight-bold text-light">
              Search bar filters items as you type. This feature searches item
              names, descriptions, and tags.
            </p>
            <p className="text-center font-weight-bold text-light">
              Items and gold totals can be filtered by character using the
              dropdown menu. Click the button to the right to add or remove
              characters.
            </p>
            <p className="text-center font-weight-bold text-light">
              To add items click the Add Item button. A name and description are
